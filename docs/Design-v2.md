...
1. **Database** — PostgreSQL (shared data store). One Postgres instance serves both the campaign finance relational data (accessed directly by the Backend) and LibreChat's document store (accessed via FerretDB, a MongoDB-compatible proxy backed by Postgres).
...
- **Cloud-portable.** No hard dependencies on a specific cloud provider. Use containers and standard services (Postgres) that run anywhere.
...
│  (may be multiple     │    Entity linking via database lookup
│   rounds)             │
└───────────┬───────────┘
            ▼
┌───────────────────────┐
...
|-------------|---------|---------------|
| **Local dev** | Individual developer machines | Docker Compose: Postgres, FerretDB, MeiliSearch, LibreChat, MCP server. Single `docker compose up` to start everything. |
...
- **Use managed Postgres** through standard connection strings, not provider-specific SDKs.
...
**Hosting costs:**
- Managed Postgres is the primary fixed cost
...
- **`docker-compose.yml` defines the full stack** including LibreChat, FerretDB, PostgreSQL, MeiliSearch, and our backend services. A developer can bring up everything with a single command.
